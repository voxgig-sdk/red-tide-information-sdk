...
class RedTideInformationMakeError
{
    public static function call(?RedTideInformationContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new RedTideInformationContext([], null);
        }
        $op = $ctx->op ?? new RedTideInformationOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new RedTideInformationResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof RedTideInformationError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new RedTideInformationError('', $msg, $ctx);
        ...
        if ($err instanceof RedTideInformationError) {
