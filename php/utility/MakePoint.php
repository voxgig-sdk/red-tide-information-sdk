...
// RedTideInformation SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class RedTideInformationMakePoint
{
    public static function call(RedTideInformationContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof RedTideInformationError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = RedTideInformationHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = RedTideInformationHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
