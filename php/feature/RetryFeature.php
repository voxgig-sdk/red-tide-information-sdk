...
// RedTideInformation SDK retry feature
...
class RedTideInformationRetryFeature extends RedTideInformationBaseFeature
{
    ...
    public function init(RedTideInformationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RedTideInformationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RedTideInformationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RedTideInformationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
