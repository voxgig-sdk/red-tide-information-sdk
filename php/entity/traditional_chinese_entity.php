...
// RedTideInformation SDK TraditionalChinese entity
...
class TraditionalChineseEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "traditional_chinese";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new TraditionalChineseEntity($this->_client, $opts);
    }

    /**
     * @param TraditionalChinese|array $args TraditionalChinese data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = RedTideInformationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return TraditionalChinese|array The current TraditionalChinese data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of TraditionalChinese fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = RedTideInformationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of TraditionalChinese fields).
     */
    ...
    /**
     * List TraditionalChinese items matching the given filter.
     *
     * @param TraditionalChineseListMatch|array|null $reqmatch Match filter (any subset
     *   of TraditionalChinese fields) as an assoc-array; TraditionalChineseListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return TraditionalChinese[]|array A list of TraditionalChinese items as assoc-arrays at
     *   the SDK boundary; throws RedTideInformationError on failure (item-5 convention).
     */
