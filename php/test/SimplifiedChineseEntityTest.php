...
class SimplifiedChineseEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->SimplifiedChinese(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->SimplifiedChinese(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set RED_TIDE_INFORMATION_TEST_SIMPLIFIED_CHINESE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $simplified_chinese_ref01_list_result = $simplified_chinese_ref01_ent->list($simplified_chinese_ref01_match, null);
...
function simplified_chinese_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("RED_TIDE_INFORMATION_TEST_SIMPLIFIED_CHINESE_ENTID");
