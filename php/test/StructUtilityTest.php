...
// RedTideInformation SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = RedTideInformationOmni::makeStructRunner(
    ...
    {
        return RedTideInformationOmni::objmodel(RedTideInformationOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return RedTideInformationOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return RedTideInformationOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(RedTideInformationOmni::structfix($now, Struct::undef()))
                        !== json_encode(RedTideInformationOmni::structfix($val, Struct::undef()))) {
