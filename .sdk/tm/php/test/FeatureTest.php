...
// RedTideInformation SDK feature test
//
...
class FtCtrl extends RedTideInformationControl
{
...
class FtClient
{
    ...
    private RedTideInformationUtility $utility;

    public function __construct(RedTideInformationUtility $utility, array $options)
    {
    ...
    public function get_utility(): RedTideInformationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RedTideInformationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RedTideInformationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RedTideInformationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RedTideInformationConfig::make_feature($name);
    ...
    private static function buildUrl(RedTideInformationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RedTideInformationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RedTideInformationResponse($fetched);
        }
    ...
    private function fail(RedTideInformationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RedTideInformationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
