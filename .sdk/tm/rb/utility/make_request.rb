...
module RedTideInformationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RedTideInformationResponse.new({})
    ...
    if fetch_err
      ...
      response = RedTideInformationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RedTideInformationResponse.new(fetched)
    else
