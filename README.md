...
This API provides updated information on red tides sighted in Hong Kong waters, including species and location.

Learn more about Red Tide Information at [www.afcd.gov.hk/data/Update_of_Red_Tide_Situation.pdf](https://www.afcd.gov.hk/data/Update_of_Red_Tide_Situation.pdf).
...
This SDK exposes the API as a small set of **semantic entities** — English, SimplifiedChinese and TraditionalChinese — that you
...
const items = await client.English().list()
```
...
$englishs = $client->English()->list();
print_r(array_map(fn($item) => $item->data_get(), $englishs));
```
