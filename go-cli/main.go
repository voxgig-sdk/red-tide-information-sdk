// Command RedTideInformation-cli is a CLI + REPL for the RedTideInformation SDK,
...
const prompt = "red-tide-information"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "english simplified_chinese traditional_chinese"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      RED_TIDE_INFORMATION_APIKEY (api key), RED_TIDE_INFORMATION_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: RED_TIDE_INFORMATION_APIKEY carries the API key and
	// RED_TIDE_INFORMATION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("RED_TIDE_INFORMATION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("RED_TIDE_INFORMATION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewRedTideInformationSDK(opts)
