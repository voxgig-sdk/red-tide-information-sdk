# red-tide-information-cli

boru-driven command-line client **and** interactive REPL for the RedTideInformation
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/red-tide-information-cli)
make build

# 2. See usage (words, entities, env vars)
./red-tide-information-cli --help

# 3. Provide credentials once, via the environment
export RED_TIDE_INFORMATION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./red-tide-information-cli list english
./red-tide-information-cli list simplified_chinese

# 5. Override the API base URL for a single call
RED_TIDE_INFORMATION_BASE=https://api.example.com ./red-tide-information-cli list english

# 6. No arguments -> interactive REPL
./red-tide-information-cli
red-tide-information> list english
red-tide-information> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/red-tide-information-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/red-tide-information-cli list english
   ```
...
Run with no arguments to open a REPL (prompt `red-tide-information>`). Each line is
evaluated as its own boru expression:

```text
$ ./red-tide-information-cli
red-tide-information> list english
red-tide-information> /help
red-tide-information> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `english`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/red-tide-information-cli`. |
...
(`./red-tide-information-cli <expr>`) and interactively (the REPL), and expressions compose the
