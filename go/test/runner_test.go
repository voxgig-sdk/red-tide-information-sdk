...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("REDTIDEINFORMATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("REDTIDEINFORMATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("REDTIDEINFORMATION_TEST_EXPLAIN"); explain != "" {
