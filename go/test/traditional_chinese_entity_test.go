...
func TestTraditionalChineseEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.TraditionalChinese(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.TraditionalChinese(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		traditionalChineseRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.traditional_chinese")))
...
func traditional_chineseBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("RED_TIDE_INFORMATION_TEST_TRADITIONAL_CHINESE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["RED_TIDE_INFORMATION_TEST_TRADITIONAL_CHINESE_ENTID"])
