...
import (
	...
	omni "github.com/voxgig-sdk/red-tide-information-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RedTideInformationError); is {
				out := map[string]any{
					"name":    "RedTideInformationError",
...
func omniCtx(arg any, client *sdk.RedTideInformationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RedTideInformationSDK); is {
			client = live
