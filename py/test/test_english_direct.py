...
class TestEnglishDirect:

    def test_should_direct_list_english(self):
        setup = _english_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-english", "live" if setup["live"] else "unit")
        if _skip:
...
def _english_direct_setup(mockres):
