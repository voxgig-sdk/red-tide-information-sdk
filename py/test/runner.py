# RedTideInformation SDK test runner
...
class RedTideInformationTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            RedTideInformationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = RedTideInformationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = RedTideInformationTestRunner.getenv("RED_TIDE_INFORMATION_TEST_LIVE")
        override = RedTideInformationTestRunner.getenv("RED_TIDE_INFORMATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = RedTideInformationTestRunner.getenv(key)
        ...
        explain = RedTideInformationTestRunner.getenv("RED_TIDE_INFORMATION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if RedTideInformationTestRunner._test_control is not None:
            return RedTideInformationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                RedTideInformationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            RedTideInformationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = RedTideInformationTestRunner.load_test_control()
...
def load_env_local():
    RedTideInformationTestRunner.load_env_local()


def env_override(m):
    return RedTideInformationTestRunner.env_override(m)


def entity_data(v):
    return RedTideInformationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return RedTideInformationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return RedTideInformationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return RedTideInformationTestRunner.load_test_control()


def live_client_options():
    return RedTideInformationTestRunner.live_client_options()


def live_delay_ms():
    return RedTideInformationTestRunner.live_delay_ms()
