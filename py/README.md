...
The Python SDK for the RedTideInformation API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.English(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.RedTideInformation(None).load(
    {"id": "test01"}, None
)
