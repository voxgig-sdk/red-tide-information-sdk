...
describe('TraditionalChineseDirect', async () => {
  ...
  test('direct-list-traditional_chinese', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-traditional_chinese', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['RED_TIDE_INFORMATION_TEST_TRADITIONAL_CHINESE_ENTID']
