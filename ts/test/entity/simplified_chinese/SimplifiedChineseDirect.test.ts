...
describe('SimplifiedChineseDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new RedTideInformationSDK({
  ...
  test('direct-list-simplified_chinese', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-simplified_chinese', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['RED_TIDE_INFORMATION_TEST_SIMPLIFIED_CHINESE_ENTID']
