...
//     (\`new RedTideInformationSDK(...)\` / \`RedTideInformationSDK.test(...)\` become
//     \`RedTideInformationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'RedTideInformationSDK'
...
const TEST_SEED = {"entity":{"english":{"test01":{"id":"test01"}},"simplified_chinese":{"test01":{"id":"test01"}},"traditional_chinese":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new RedTideInformationSDK(...)\` and any
// \`RedTideInformationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RedTideInformationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RedTideInformationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
