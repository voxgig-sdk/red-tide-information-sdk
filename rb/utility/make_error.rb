...
module RedTideInformationUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = RedTideInformationContext.new({}, nil)
    end
    op = ctx.op || RedTideInformationOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || RedTideInformationResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(RedTideInformationError) ? err.msg : err.to_s
    ...
    sdk_err = RedTideInformationError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(RedTideInformationError)
