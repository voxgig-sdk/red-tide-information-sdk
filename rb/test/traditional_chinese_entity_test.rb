...
class TraditionalChineseEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.TraditionalChinese(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.TraditionalChinese(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set RED_TIDE_INFORMATION_TEST_TRADITIONAL_CHINESE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def traditional_chinese_basic_setup(extra)
  ...
  entid_env_raw = ENV["RED_TIDE_INFORMATION_TEST_TRADITIONAL_CHINESE_ENTID"]
