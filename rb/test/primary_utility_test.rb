...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = RedTideInformationOmni.make_runner(TEST_JSON_FILE, RedTideInformationSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    RedTideInformationError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : RedTideInformationOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = RedTideInformationOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
