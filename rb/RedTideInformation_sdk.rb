...
require_relative 'RedTideInformation_types'
...
class RedTideInformationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = RedTideInformationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = RedTideInformationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, RedTideInformationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    RedTideInformationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = RedTideInformationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = RedTideInformationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = RedTideInformationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => RedTideInformationError.new(
        "#{op}_allow",
        "RedTideInformationSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue RedTideInformationError => err
    ...
    if fetched.is_a?(Hash)
      status = RedTideInformationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = RedTideInformationError.new(
        "graphql_error", "RedTideInformationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = RedTideInformationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
