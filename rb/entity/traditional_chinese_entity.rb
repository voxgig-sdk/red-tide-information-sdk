...
class TraditionalChineseEntity
  ...
  def make
    opts = @_entopts.dup
    TraditionalChineseEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = RedTideInformationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [TraditionalChinese, Hash] the current TraditionalChinese data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = RedTideInformationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of TraditionalChinese fields)
  def match_get
  ...
  #
  # @param reqmatch [TraditionalChineseListMatch, Hash, nil] match filter (any subset of
  #   TraditionalChinese fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<TraditionalChinese>, Array] the matching TraditionalChinese items; raises RedTideInformationError on failure
