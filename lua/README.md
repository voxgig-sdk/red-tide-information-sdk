...
The Lua SDK for the RedTideInformation API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:English(nil):list(nil, nil)
...
local result, err = client:RedTideInformation(nil):load(
  { id = "test01" }, nil
)
