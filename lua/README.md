# RedTideInformation Lua SDK



The Lua SDK for the RedTideInformation API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:English()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```
lua/
├── red-tide-information_sdk.lua    -- Main SDK module
...
The main module (`red-tide-information_sdk`) exports the SDK constructor
