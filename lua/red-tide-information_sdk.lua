...
require("red-tide-information_types")
...
local RedTideInformationSDK = {}
RedTideInformationSDK.__index = RedTideInformationSDK
...
RedTideInformationSDK._make_feature = _make_feature


function RedTideInformationSDK.new(options)
  local self = setmetatable({}, RedTideInformationSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function RedTideInformationSDK:options_map()
...
function RedTideInformationSDK:get_utility()
  return Utility.copy(self._utility)
end


function RedTideInformationSDK:get_root_ctx()
  return self._rootctx
end


function RedTideInformationSDK:prepare(fetchargs)
...
function RedTideInformationSDK:direct(fetchargs)
...
function RedTideInformationSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function RedTideInformationSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "RedTideInformationSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function RedTideInformationSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function RedTideInformationSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "RedTideInformationSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function RedTideInformationSDK:English(data)
  ...
  if data == nil then
    if self._english == nil then
      self._english = EntityMod.new(self, nil)
    end
...
function RedTideInformationSDK:SimplifiedChinese(data)
  ...
  if data == nil then
    if self._simplified_chinese == nil then
      self._simplified_chinese = EntityMod.new(self, nil)
    end
    return self._simplified_chinese
  end
  return EntityMod.new(self, data)
end
...
function RedTideInformationSDK:TraditionalChinese(data)
  ...
  if data == nil then
    if self._traditional_chinese == nil then
      self._traditional_chinese = EntityMod.new(self, nil)
    end
...
function RedTideInformationSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = RedTideInformationSDK.new(sdkopts)
  sdk.mode = "test"
