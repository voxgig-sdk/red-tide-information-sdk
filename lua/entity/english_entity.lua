...
local EnglishEntity = {}
EnglishEntity.__index = EnglishEntity


function EnglishEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EnglishEntity)
  self._name = "english"
  self._client = client
...
function EnglishEntity:get_name()
  return self._name
end


function EnglishEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EnglishEntity.new(self._client, opts)
end
...
function EnglishEntity:mark_deleted()
  self._deleted = true
end


function EnglishEntity:deleted()
  return true == self._deleted
end


function EnglishEntity:data_set(args)
  if args ~= nil then
...
function EnglishEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EnglishEntity:match_set(args)
  if args ~= nil then
...
function EnglishEntity:match_get()
...
function EnglishEntity:stream(action, args, callopts)
...
function EnglishEntity:_run_op(ctx, post_done)
  local utility = self._utility
