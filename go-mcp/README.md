...
[MCP](https://modelcontextprotocol.io) server exposing the RedTideInformation SDK as
two agent tools — `red-tide-information_list` and `red-tide-information_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/red-tide-information-mcp)
make build

# 2. Provide credentials via the environment
export RED_TIDE_INFORMATION_APIKEY=sk_live_xxx
...
```jsonc
// red-tide-information_list: first page of records
{ "entity": "english" }
{ "entity": "english", "query": { } }

// red-tide-information_load: one record by id
{ "entity": "english", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/red-tide-information-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user red-tide-information \
     -- "$PWD"/dist/*/red-tide-information-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `red-tide-information_list` and `red-tide-information_load` tools now appear
   in new sessions. Ask the agent to *"list english using red-tide-information"*
   and it calls `red-tide-information_list` with `{"entity":"english"}`.
...
```sh
export RED_TIDE_INFORMATION_APIKEY=sk_live_xxx            # API key
export RED_TIDE_INFORMATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `red-tide-information_list` tool
...
### Call the `red-tide-information_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `red-tide-information_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `red-tide-information_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `RED_TIDE_INFORMATION_APIKEY` | API key sent with every request. |
| `RED_TIDE_INFORMATION_BASE` | Optional override of the API base URL. |
